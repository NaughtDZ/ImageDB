...
- **右键操作**：重新扫描（标记丢失并询问是否清理）/ 整个目录打标 / 添加图片到该目录 / 检查目录 / **从库中移除（仅删记录，磁盘文件不动）**；
...
### 🧩 元数据/正则打标（实验）
- **零模型**：不靠 AI，而是按你写的**正则规则**，从**文件名**或文件的**附加元数据（EXIF/IPTC/XMP）**里直接提取标签，毫秒级、免成本；
- 每条规则可设 `source`(filename|metadata|all)、`pattern`(正则)、`flags`、`tag` 模板(`{match}`、`$1..$9`、`{name}`)、`normalize`；
- 支持单图/多选/整个目录打标；标签以 `source=metadata` 写入，可与其它来源区分管理；
- 配置见设置页「打标」tab 的「元数据/正则打标（实验）」**规则 JSON** 编辑器，保存后插件自动重载；
- 📖 **零基础图文教程**见 [`docs/metadata_tagging_guide.md`](docs/metadata_tagging_guide.md)。
...
### 🧹 外部丢失不删记录（占位 + 手动清理）
- **外部丢失 ≠ 删记录**：文件/目录被外部删除（含移动盘拔掉）后，记录**保留**并标记 `status=missing`；标签、缩略图一并保留，磁盘接回来会**自动恢复**为正常；
- 画廊里丢失的媒体显示**「丢失」占位图 + 角标**（占位图为前端内置 SVG）；点击/查看时提示「**图片路径 XXX 已丢失**」；丢失目录在树里标 🚫（**点击该节点时才判定并就地标注**——目录树保持纯数据库读取，不逐目录 stat，避免网络盘上启动变慢）；
- 只有**用户主动删**（🗑 删除选中 / 移回收站 / 移出库）才会真正删除数据库记录；
- **清理丢失记录需显式确认**：右键目录「重新扫描」（或顶栏「🔎 扫描丢失」扫描）后弹窗显示「**新增 N 个 · 发现丢失 M 个 · 恢复 K 个**」，由你决定**删除丢失记录**还是**保留**；
- 顶栏「🔎 扫描丢失」= 全库扫描（**只标记、不删除**）。

**怎么"知道"文件丢了？（没有实时监视）**
- **不做文件系统实时监视**（那需要为每个目录挂句柄，网络盘不可靠、开销大），改为三种**按需**方式：
  1. **浏览时按需检测**：你翻到哪张，就对该条做一次 `stat` 同步状态（请求缩略图时顺带完成，几乎零额外开销）；
  2. **手动扫描**：顶栏「🔎 扫描丢失」（全库）或右键目录「重新扫描」（该子树）；
  3. **可选后台定时**：设置 → 视频页「后台自动标记丢失间隔」，**默认 0=关闭**（定时全量重扫对网络盘/大库开销过大）；想自动发现可设 600~1800 秒。
- **「盘没挂」≠「文件丢了」**：根目录不可达（移动盘拔掉 / 网络盘掉线）时整棵子树记为**离线**（树里 💤、不标 missing、**不删记录**），避免拔盘就把几十万条误标为丢失；盘接回后恢复正常。真要放弃该盘，用右键「从库中移除」显式清理。
...
6. **维护**：顶栏「🔎 扫描丢失」全库扫描（只标记丢失，不删记录）；清理需右键目录「重新扫描」后确认；后台定时也会自动标记；
...
- 📖 **零基础图文教程**见 [`docs/metadata_tagging_guide.md`](docs/metadata_tagging_guide.md)；🤖 想让 AI 帮你生成提取规则？用 [`docs/metadata_tagging_prompt.md`](docs/metadata_tagging_prompt.md) 这份提示词。
