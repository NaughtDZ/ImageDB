...
- **后台执行 + 进度条**：导出 / 导入 / 迁移自检都在**后台线程**跑（可能涉及上万个目录的网络盘读写），点击后弹窗**立刻显示进度条**（当前目录 + `已完成/总数`），不会「点了没反应」；关掉弹窗即停止轮询，任务仍在后台继续，可用 `GET /api/tags/jobs` 查看；同一时间只允许一个 `.imgtag` 任务，重复点击会提示「已有任务在进行中」；
...
### 🧹 外部丢失不删记录（占位 + 手动清理）
> **核心原则：程序只做「恢复」，绝不做「标记」；「丢失」只能由你显式重扫后确认，「删除」只能由你显式点。**

- **外部丢失 ≠ 删记录**：文件/目录被外部删除（含移动盘拔掉）后，**数据库记录一律保留**——标签、缩略图、`status` 都不动；
- **只有两个地方会改状态**：
  1. **你手动重扫**某个目录（顶栏「⟳ 重新扫描」或右键目录）：核对磁盘后弹窗显示「**新增 N 个 · 发现丢失 M 个 · 恢复 K 个**」，由你决定**删除丢失记录**还是**保留**（不删＝记录/标签/缩略图全留着，盘接回来自动恢复）；
  2. **你主动删**（🗑 删除选中 / 移回收站 / 移出库）：直接删记录；
- **浏览/看图的判定是「现场事实」，不写库**：翻到哪张就现场读一次——
  - 读得到 → 正常显示（顺手把过时的「丢失」标记恢复成正常，审计原因 `access:present`）；
  - 读不到（确认文件不在）→ 缩略图**优先给缓存图**（右上角「⚠ 丢失」角标），没有缓存才给占位图；双击查看时提示「**图片路径 XXX 已丢失**」；目录被删同理在树里标 🚫 + 提示路径；
- **「盘没挂」≠「文件丢了」**：只有**确认不存在**（`FileNotFoundError`）才算丢失；权限不足 / 网络抖动 / 盘掉线一律算**未知** → **不动数据库**（所以不会出现"一启动一堆图报丢失"）。根目录不可达时整棵子树记为**离线**（树里 💤，不标记、不删除）；真要放弃该盘，用右键「从库中移除」显式清理；
- **顶栏「♻ 恢复校验」**：一键检查所有被标记为「丢失」的记录，把**实际还在磁盘上**的恢复回正常；读不到的计入「无法确认」并保持原状。只恢复、不标记、不删除；
- **审计字段**：每次状态变化都记录 `status_at`（时间）与 `status_reason`（原因，如 `rescan:not_found` / `rescan:present` / `recheck:recovered` / `access:present`），排查时能直接看出「什么时候、为什么被标的丢失」；
- **「⚠ 只看丢失」筛选器**（工具栏）：一键只列出被标记为丢失的媒体；按钮上带**当前范围的丢失数量**角标（选了目录=该目录子树，未选=全库）；再点一次关闭。可与目录/文件名/标签/类型筛选叠加。

**怎么"知道"文件丢了？（没有实时监视，也没有定时全库扫描）**
- **不做文件系统实时监视**（网络盘上挂句柄不可靠、开销大），也**取消了后台定时全库扫描**（对网络盘/大库开销过大，而且正是误判的来源）；
- 判定完全**按需**：浏览缩略图 / 打开原图时现场读一次（只读探测，失败不改库），外加你手动「重新扫描」的那一次。
- 目录树本身仍是**纯数据库读取**（绝不逐目录 stat）；点节点只做一次**只读**存在性探测，不改库、不标记录。
- **根目录探测有超时保护**：只探测根目录（通常 1~4 个），且带 **1.5 秒超时 + 20 秒缓存**——网络盘卡死也不会阻塞目录树加载；用户主动扫描时缓存自动失效重新探测。
...
6. **维护**：顶栏「♻ 恢复校验」把误标的丢失恢复回正常；要**标记**丢失必须手动右键目录「重新扫描」并在弹窗中确认；已无后台定时扫描；
...
2. **迁移自检**（可选）：顶栏「🔍 迁移自检」核对 **主库媒体 vs 磁盘 vs `.imgtag`** —— 缺 `.imgtag` 的目录 / `.imgtag` 孤儿引用 / 未覆盖的媒体；迁移前看一眼更安心。
   ...
   > 导出 / 导入 / 自检均已改为**后台任务 + 进度条**：弹窗立即出现进度条（当前目录 + 已完成/总数），跑完自动展示结果。网络盘上目录很多时尤其明显。
...
- **改了代码但没生效？** 设置 → **关于** 里看「代码版本 / 代码时间 / 本次进程启动」：若**代码时间晚于进程启动时间**，说明当前进程还是旧代码（刷新网页只更新前端，不会重载 Python 后端），**重启程序**即可；
